...
- **`store_open`:** creates parent dirs `0700`, applies `foreign_keys=ON` + `busy_timeout=5000`, gates on `user_version` (0→create at **4**, 1/2/3→migrate to 4, 4→ok, >4→error `"database is newer than this remember"`). Sidecar `<path>.device_id` (mode 0600); `devices` at most one local row; no `last_seen` bump on get/list. Default journal (no WAL). **No `expires_at` index** (ponytail; personal scale).
- **Schema bootstrap must be atomic:** run initial DDL + `user_version=4` inside one transaction (or tear down on failure). A partial create at `user_version=0` bricks later opens (`table already exists`). v1 migrate is `ALTER TABLE entries ADD COLUMN expires_at TEXT` plus `entry_links` DDL plus v4 columns; v2 migrate is `entry_links` plus v4 columns; v3 migrate is v4 columns (`sync_id`/`deleted_at`/`version_vector` + `devices`/`conflicts`) — same write-lock transaction.
- **`entry_links` (P7, store stage):** one row per `(from_id, to_id, kind)` with stored kind `related|supersedes|cites`. `related` is canonical `(min,max)`. No `dir` column, no self-heal. Reads are `from_id = S OR to_id = S`. Ports: `store_link` / `store_unlink` / `store_list_neighbors` / `store_list_neighbors_for` / `store_get_any` / `store_rekey`. Output `cited_by` is CLI, not store. Real graph writes bump both endpoint `updated_at`. `store_rekey` set/rename/clear in place (id + edges kept); empty new key is INTERNAL (CLI rejects `empty key`). `supersedes_reaches` is recursive set **`UNION`** (not `UNION ALL`). Neighbor row copies fail closed on OOM (never emit a keyless/durable lie); unlink `fill_stub` failures keep their real status.
...
- **Commands layer:** public `commands.h` only. Implementation is split by **concern axis**, not one file per subcommand:
  - `commands_common.c` / `commands_common.h` — internal shared helpers (err messages, tag normalize, argv helpers, `load_body`, `--ttl`/`--expires` parse). **Not** a public API; only `cmd_*.c` include it.
  - `cmd_add.c` — write path (key/tags, `store_add`; body via shared `load_body`; optional expiry).
  - `cmd_locator.c` — get + delete + update + rekey (shared id/`--key`/`--sync-id` locator; update adds `--text`/`--tag`/`--clear-tags`/`--ttl`/`--expires`/`--clear-expires`/`--undelete`; `--expired`/`--deleted`/`--trash` via `CmdBinOpts`). Unflagged delete is soft (live or expired); `--expired`/`--deleted` on delete is one-row hard. `--sync-id` must be canonical lowercase UUID v7.
  - `cmd_query.c` — list + search (shared filter/paging parse; different store call; same bin flags).
  - `cmd_purge.c` — `purge --expired|--deleted` (exactly one flag; whole-bin hard wipe) and `purge-trash` (deprecated alias of `purge --expired`). Wire in `REMEMBER_LIB_SOURCES` **and** the `-Wcast-qual` list.
  - `cmd_graph.c` — `link` / `unlink` / `related` (pair locators id/`--*-key`/`--*-sync-id`; default either-bin live+expired via `store_get_any*`; `--deleted` expands to `store_get_row*` so deleted ends resolve; `--expired` rejected on graph). 5-name `type` is output, not store. `rekey` lives in `cmd_locator.c` (update’s bin contract). Graph resolvers must return `REMEMBER_ERR` (1), never `-1` (that becomes exit 255). Positional `ID ID` sugar is `--from`/`--to` on **link and unlink** (numeric ids only). link/unlink/rekey bump VV on affected ends; no-op unlink does not.
  - `cmd_import.c` — `import --from-db`, `conflicts`, `conflict accept --id --keep`. Store owns RO open + merge; commands never open a second SQLite handle.
...
- **Output:** `output.c` owns JSON escaping (RFC 8259) and envelopes; human `add` prints id only; human `get` uses `output_body_human` (terminal C0/DEL → `?`, keep `\n`/`\t`) then an optional `Related:` block; human list previews also neutralize controls. Never raw-`fputs` a stored body to a TTY. Entry JSON field order is `id,sync_id,key,body,tags,source,created_at,updated_at,expires_at,deleted_at,bin,version_vector[,links]`. `bin` is always `"live"|"expired"|"deleted"` (never null, never omit). No `"trash"` field. Stubs are `{id,sync_id,key,type,bin,preview}`. list/search/get JSON append `"links"` after `"version_vector"` (empty array if none). add/update/delete/purge JSON stay without `links`. Human list/search sixth column is neighbor **ids** only (cap 5 then `, +N`); suffix `[expired]` / `[deleted]` (never `[trash]`). Stub `type` is the 5-name vocabulary; stored kind stays 3. Default related/stubs omit deleted neighbors; expired neighbors stay visible. `--deleted` includes them.
...
- **In-process façade (step 11):** the native GUI links the core instead of spawning the process (MAS sandbox can't spawn to reach `~/.remember`). One public entrypoint `remember_run(argc, argv, out, err)` in `remember_app.c` (`remember_app.h`) — the CLI pipeline minus `main`, so its `--json` bytes are identical to the CLI by construction (no parallel orchestration). The GUI builds argv incl. `--db <bookmarked path> --json`. `remember_main` wraps `remember_run` with stdout/stderr so `main.c` stays include-free (also dodges a Linux clang-tidy misc-include-cleaner quirk on `<stdio.h>`). Design log 002.
  - **Stream injection (`appio.c`):** command/help/error paths write through `app_out()`/`app_err()` (process-global pair, default stdout/stderr; `remember_run` swaps around one call). `ponytail:` global, not threaded — CLI is single-threaded and the GUI serializes via an actor. **Rule:** never `fprintf(stdout/stderr, …)` or `output_*(stdout, …)` directly in command code; use `app_out()`/`app_err()`. `output.c` is unchanged (already `FILE*`-parameterized).
  - **`libremember` (`remember_core` target):** `REMEMBER_LIB_SOURCES` minus `main.c`; the only symbol a consumer needs is `remember_run`. Only `store_sqlite.c` includes sqlite still holds.
  - **`tags` command:** `store_tags` (distinct names + counts, INNER JOIN so only in-use tags appear, sorted `COLLATE BINARY`) → `output_tags_envelope`/`output_tags_human` → `cmd_tags`. Envelope `{"version":1,"count":N,"tags":[{"name","count"}]}`. Added for GUI tag chips + add-form autocomplete; on the CLI too so surfaces stay in sync.
- **Facade parity test (`test_facade.c`):** asserts `remember_run` stdout/stderr/exit byte-match the CLI subprocess for the same argv. Reads compare exact bytes; mutations mask `created_at`/`updated_at`/`deleted_at`/`sync_id`/`version_vector` (they legitimately differ between two writes). Guards the stream injection against future drift. Stage 6 covers `import` / `conflicts` / `conflict accept` in addition to earlier commands.
- **Sync-path warning (step 09):** `util_path_looks_synced()` — substring markers `com~apple~CloudDocs`, `Dropbox`, `Google Drive`. Called from `main` after resolve, before `store_open`; one-line stderr; still opens. Store stays path-pure (no warning logic in adapter).
- **Format target (step 08):** CMake always defines `format`. On Apple, configure prepends `$(brew --prefix llvm)/bin` to `CMAKE_PROGRAM_PATH` so `clang-format` is found without a manual PATH. Reconfigure after installing llvm if the target previously reported missing.
- **Install (step 10):** `scripts/install.sh` builds/installs from a dedicated **Release** tree (`build-release/` by default, `REMEMBER_RELEASE_DIR`). It never reuses developer `build/` (often ASan) and refuses to install a binary that links asan/ubsan. `--force-build` cleans only that Release tree. Skill install is `scripts/remember-install-skill` (symlink by default; `--copy` for clone installs). Destinations: existing `~/.grok` / `~/.claude` / `~/.cursor` skill trees, or `REMEMBER_SKILL_DIRS`. CMake installs the binary, the helper, and `share/remember/SKILL.md` — it does not write `$HOME` (Homebrew cannot).
- **User-facing strings are ASCII** (help, stderr, warnings). No Unicode dashes/arrows in CLI messages.
- **Sync markers tested:** unit coverage for `com~apple~CloudDocs`, `Dropbox`, `Google Drive` plus black-box stderr warnings for each (`tests/test_schema_config.c`, suite `schema_config`).
- **`store_add`:** keyless body-hash merge (union tags, keep source/created_at/body/`sync_id`) or keyed upsert (replace body, union tags, keep source/created_at/key/`sync_id`). Hitting an expired **or deleted** row revives it (clear `deleted_at`, apply incoming expiry or none, bump VV). FTS resync in the same transaction.
...
- **Soft vs hard delete (two functions; hard cannot default):** `store_soft_delete_by_id`/`_by_key` (live or expired) sets `deleted_at=now`, clears `expires_at`, keeps body/tags/key/`sync_id`/edges/FTS, bumps VV. `store_hard_delete_by_id`/`_by_key` requires `StoreBin` EXPIRED or DELETED (LIVE → INTERNAL), `devices` COUNT==1 else `STORE_ERR_NOT_SINGLE_DEVICE`, FTS delete then `DELETE entries` (CASCADE links/tags) + orphan-tag GC. Unflagged CLI `delete` is soft; `delete --expired|--deleted` (and `--trash` ≡ `--expired`) is one-row hard. JSON `action:deleted` still names the snapshot.
- **Locator (get/delete/update/rekey):** shared validate/resolve in `cmd_locator.c` — exactly one of positional id / `--key` / `--sync-id` (canonical v7); reject `--source`. Unflagged delete locators are live **or** expired; other default locators stay live-only. Update adds its own flag parse (`--text`/`--tag`/`--clear-tags`/`--undelete`) on top of the same locator contract.
- **Stable exit codes:** `0` ok, `1` usage/error, `2` not found (neither bin), **`3` wrong bin**. Tokens (Round 7): `expired` | `deleted` | `not_expired` | `not_deleted`. `REMEMBER_WRONG_BIN = 3`. `store_status_to_exit` maps those four statuses to 3. Locators take `StoreBin` + `now`. `--expired`/`--deleted` are mutex; `--trash` aliases `--expired` with one stderr deprecation line per invocation.
...
- **CLI search:** same filter/paging parse path as list (`cmd_query.c`) + one required QUERY positional. Outer ASCII whitespace is trimmed (same set as normalize: space/tab/LF/CR/VT/FF); empty-after-trim → usage error `"empty search query"` (not FTS). Trimmed string is the FTS MATCH input (not tag/key-normalized). Human preview via `output_entry_human_line` (≤80 codepoints, plus related-id cell); JSON full bodies + link stubs via `output_list_envelope`. Page neighbors loaded with one `store_list_neighbors_for`.
- **`store_update`:** locate by id or key; independent `set_body` / `set_tags` / `set_expires` / `undelete` flags; always bump `updated_at` on success; FTS resync in the same write txn. `undelete` clears `deleted_at` and bumps VV (CLI mutex with `--ttl`/`--expires`/`--clear-expires`; locator normally `--deleted`). Keyless body-hash collision → `STORE_ERR_CONFLICT` + conflicting id (CLI prints it on stderr). Tag replace deletes `entry_tags` then re-links + orphan GC. Never changes `source` or `key`.
- **CLI update:** lives in `cmd_locator.c` (same id/`--key` contract as get/delete) + opt-in `--text` / `--tag` / `--clear-tags` / `--undelete`. Body load via shared `load_body` in `commands_common` (add + update). No positional body; `--source` rejected.
- **`store_purge`:** whole-bin hard wipe of EXPIRED or DELETED (LIVE → INTERNAL); same COUNT==1 hatch as one-row hard. `store_purge_trash` is the EXPIRED wrapper. CLI `purge` requires exactly one of `--expired`|`--deleted`; `purge-trash` ≡ `purge --expired` + deprecation stderr.
- **Import / conflicts (plan 14 stage 5, Round 7):** `store_import` opens the foreign path read-only inside the adapter (never `store_open`, never sidecar I/O, never INSERT foreign `devices`). `user_version` < 4 → `STORE_ERR_SOURCE_TOO_OLD` (`source database is older than this remember`). Merge by `sync_id`: missing → insert preserving sync_id/VV (unique key/hash clash → `conflicts` row); present → VV dominate / identical auto-merge / concurrent → `concurrent_vv` (pairwise-max local VV first; open conflict upserted by `(sync_id, reason)` — `decision:sync-import-reimport-idempotent`). **Clash re-import uses the same upsert** (`upsert_conflict`): a second import of the same `key_clash`/`hash_clash` refreshes snapshots and does not insert another row (envelope `conflicts` counts only new rows). Accept always unions incoming VV into survivors; `--keep both` remints sync_id on `concurrent_vv` (`{local:1}`), preserves incoming sync_id on clash (keyless if slot taken). Keyed clash: keep local key, demote imported until accept; never invent `key=sync_id` — contested keyless hash → `STORE_ERR_UNIQUE_TAKEN` (`decision:sync-import-hash-demote`). `--keep incoming` on `concurrent_vv` demotes any *other* unique occupant before apply (same hatch — never raw SQLITE unique). Reason tokens live in one place: `store_conflict_reason_str`. CLI: `import --from-db`, `conflicts`, `conflict accept --id N --keep local|incoming|both`. JSON import envelope: `action:imported` + inserted/updated/unchanged/conflicts counts (exit 0 even with conflicts). `VV_OUT_MAX` is 4096 so pairwise-max can absorb foreign device keys without registering them.
