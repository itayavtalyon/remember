...
- **Commands layer:** public `commands.h` only. Implementation is split by **concern axis**, not one file per subcommand:
  ...
  - `cmd_locator.c` — get + delete + update (shared id/`--key` locator; update adds `--text`/`--tag`/`--clear-tags`/`--ttl`/`--expires`/`--clear-expires`; `--expired`/`--deleted`/`--trash` via `CmdBinOpts`).
  - `cmd_query.c` — list + search (shared filter/paging parse; different store call; same bin flags).
  - `cmd_purge.c` — `purge-trash` (no locator; one store call). Wire in `REMEMBER_LIB_SOURCES` **and** the `-Wcast-qual` list.
...
- **Facade parity test (`test_facade.c`):** asserts `remember_run` stdout/stderr/exit byte-match the CLI subprocess for the same argv. Reads compare exact bytes; mutations mask `created_at`/`updated_at`/`sync_id`/`version_vector` (they legitimately differ between two writes). Guards the stream injection against future drift.
