...
- **Facade parity test (`test_facade.c`):** asserts `remember_run` stdout/stderr/exit byte-match the CLI subprocess for the same argv. Reads compare exact bytes; mutations mask `created_at`/`updated_at`/`deleted_at`/`sync_id`/`version_vector` (they legitimately differ between two writes). Guards the stream injection against future drift.
