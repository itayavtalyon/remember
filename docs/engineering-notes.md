...
- **`store_open`:** creates parent dirs `0700`, applies `foreign_keys=ON` + `busy_timeout=5000`, gates on `user_version` (0→create at 2, 1→migrate to 2, 2→ok, >2→error `"database is newer than this remember"`). Default journal (no WAL). **No `expires_at` index** (ponytail; personal scale).
- **Schema bootstrap must be atomic:** run initial DDL + `user_version=2` inside one transaction (or tear down on failure). A partial create at `user_version=0` bricks later opens (`table already exists`). v1 migrate is `ALTER TABLE entries ADD COLUMN expires_at TEXT` in the same write-lock transaction.
