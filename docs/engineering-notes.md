...
- **Stable exit codes:** `0` ok, `1` usage/error, `2` not found (neither bin), **`3` wrong bin** (`expired` / `not_expired`; stage 1 also maps `deleted` / `not_deleted`). `REMEMBER_WRONG_BIN = 3`. `store_status_to_exit` maps those four statuses to 3. Locators take `StoreBin` + `now`. JSON still has `"expires_at"` after `"updated_at"` (stage 2 adds `bin` / `sync_id` field order).
