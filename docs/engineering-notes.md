...
- **`store_open`:** creates parent dirs `0700`, applies `foreign_keys=ON` + `busy_timeout=5000`, gates on `user_version` (0→create at 3, 1→migrate to 3, 2→migrate to 3, 3→ok, >3→error `"database is newer than this remember"`). Default journal (no WAL). **No `expires_at` index** (ponytail; personal scale).
- **Schema bootstrap must be atomic:** run initial DDL + `user_version=3` inside one transaction (or tear down on failure). A partial create at `user_version=0` bricks later opens (`table already exists`). v1 migrate is `ALTER TABLE entries ADD COLUMN expires_at TEXT` plus `entry_links` DDL; v2 migrate is `entry_links` only — same write-lock transaction.
...
- **Commands layer:** public `commands.h` only. Implementation is split by **concern axis**, not one file per subcommand:
  ...
  - `cmd_locator.c` — get + delete + update (shared id/`--key` locator; update adds `--text`/`--tag`/`--clear-tags`/`--ttl`/`--expires`/`--clear-expires`; `--trash` on locators).
  - `cmd_query.c` — list + search (shared filter/paging parse; different store call; `--trash`).
...
- **Output:** `output.c` owns JSON escaping (RFC 8259) and envelopes; human `add` prints id only; human `get` uses `output_body_human` (terminal C0/DEL → `?`, keep `\n`/`\t`) then an optional `Related:` block; human list previews also neutralize controls. Never raw-`fputs` a stored body to a TTY. list/search/get JSON append `"links"` after `"expires_at"` (stubs, empty array if none). add/update/delete/purge JSON stay without `links`. Human list/search sixth column is neighbor **ids** only (cap 5 then `, +N`). Stub `type` is the 5-name vocabulary; stored kind stays 3.
...
- **Facade parity test (`test_facade.c`):** asserts `remember_run` stdout/stderr/exit byte-match the CLI subprocess for the same argv. Reads compare exact bytes; mutations mask `created_at`/`updated_at` (they legitimately differ between two writes). Guards the stream injection against future drift.
...
- **Stable exit codes:** `0` ok, `1` usage/error, `2` not found (neither bin), **`3` wrong bin** (`expired` / `not_in_trash`). `REMEMBER_WRONG_BIN = 3`. `store_status_to_exit` maps `STORE_ERR_EXPIRED` / `STORE_ERR_NOT_IN_TRASH` to 3. Store APIs that compare expiry take `const char *now`; CLI calls `utc_now()` once per command (no clock port). Trash iff `expires_at IS NOT NULL AND expires_at <= now` (lexicographic ISO `.mmmZ`). JSON always includes `"expires_at"` after `"updated_at"` (`null` or ISO). list/search/get also include `"links"` after `"expires_at"`. Human list/get do not print expiry.
