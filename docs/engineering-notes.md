...
- **Commands layer:** public `commands.h` only. Implementation is split by **concern axis**, not one file per subcommand:
  ...
  - `cmd_locator.c` — get + delete + update (shared id/`--key` locator; update adds `--text`/`--tag`/`--clear-tags`/`--ttl`/`--expires`/`--clear-expires`/`--undelete`; `--expired`/`--deleted`/`--trash` via `CmdBinOpts`). Unflagged delete is soft (live or expired); `--expired`/`--deleted` on delete is one-row hard.
  ...
  - `cmd_graph.c` — `link` / `unlink` / `related` (pair locators, either-bin `store_get_any`; 5-name `type` is output, not store). `rekey` lives in `cmd_locator.c` (update’s bin contract). Graph resolvers must return `REMEMBER_ERR` (1), never `-1` (that becomes exit 255). Positional `ID ID` sugar is `--from`/`--to` on **link and unlink**.
...
- **Locator (get/delete/update):** shared validate/resolve in `cmd_locator.c` — exactly one of positional id / `--key`; reject `--source`. Unflagged delete locators are live **or** expired; other default locators stay live-only. Update adds its own flag parse (`--text`/`--tag`/`--clear-tags`/`--undelete`) on top of the same locator contract.
