...
- **Install (step 10):** `scripts/install.sh` builds/installs from a dedicated **Release** tree (`build-release/` by default, `REMEMBER_RELEASE_DIR`). It never reuses developer `build/` (often ASan) and refuses to install a binary that links asan/ubsan. `--force-build` cleans only that Release tree. Skill copy: `skills/remember/SKILL.md` into product skill roots (or `REMEMBER_SKILL_DIRS`). CMake `install(TARGETS remember)` is binary packaging only.
