...
static const CommandEntry k_commands[] = {
    ...
    {.name = "add", .command = CLI_CMD_ADD, .summary = "Store a memory (optional --key, --tag, --source)"},
    ...
    {.name = "update", .command = CLI_CMD_UPDATE, .summary = "Change body and/or tags by id or --key"},
    ...
    {.name = "purge-trash", .command = CLI_CMD_PURGE_TRASH, .summary = "Permanently delete every expired memory"},
    ...
    {.name = "rekey", .command = CLI_CMD_REKEY, .summary = "Rename, set, or clear an entry key in place"},
};
